...
def run_diagnostics(planet, channel, AOR_snip, basepath, addStack, ncpu=4, nsigma=3):
    ...
    savepath = basepath+planet+f'/analysis/frameDiagnostics/{channel}/'
    if addStack:
        savepath += 'addedStack/'
    else:
        savepath += 'addedBlank/'
    ...
    flux, flux_err, time, xo, yo, xw, yw, bg, bg_err, npp = get_lightcurve(datapath, savepath, AOR_snip, channel,
                                                                           save = False, bin_data = False, plot = False,
                                                                           oversamp=False, planet=planet, rs = [2.5],
    ...
    flux_other_markers = np.ma.concatenate((bg_markers, xdata_markers, ydata_markers, xw_markers, yw_markers, npp_markers))
    ...
    axes[4].set_ylim(np.ma.min([(np.ma.min(xw_med)-1)*4/3+1, 1-(nsigma+1)*sigmaxw]), np.ma.max([(np.ma.max(xw_med)-1)*4/3+1, 1+(nsigma+1)*sigmaxw]))
    axes[5].set_ylim(np.ma.min([(np.ma.min(yw_med)-1)*4/3+1, 1-(nsigma+1)*sigmayw]), np.ma.max([(np.ma.max(yw_med)-1)*4/3+1, 1+(nsigma+1)*sigmayw]))
    axes[6].set_ylim(np.ma.min([(np.ma.min(npp_med)-1)*4/3+1, 1-(nsigma+1)*sigmanpp]), np.ma.max([(np.ma.max(npp_med)-1)*4/3+1, 1+(nsigma+1)*sigmanpp]))
